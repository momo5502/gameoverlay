...
namespace gameoverlay::dxgi
{
    namespace
    {
        ...
        void translate_vertices(ID3D10Buffer& vertex_buffer, const int32_t x, const int32_t y, const dimensions dim,
    ...
    dimensions d3d10_canvas::get_dimensions() const
    {
        return this->dimensions_;
    }

    void d3d10_canvas::resize(const dimensions dim)
    {
        if (this->dimensions_ == dim)
        {
            return;
        }

        this->dimensions_ = dim;
        this->texture_ = create_texture_2d(*this->device_, dim, DXGI_FORMAT_R8G8B8A8_UNORM);
    ...
    void d3d10_canvas::paint(const std::span<const uint8_t> image)
    {
        ...
        else if (desc.Usage == D3D10_USAGE_DEFAULT)
        {
            ...
            this->device_->UpdateSubresource(this->texture_, 0, &box, image.data(), row_pitch, image.size());
        }
    }
    ...
    void d3d10_canvas::draw() const
    {
        ...
        translate_vertices(*this->vertex_buffer_, 0, 0, this->dimensions_, ~0);
