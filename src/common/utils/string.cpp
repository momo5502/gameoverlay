...
namespace utils::string
{
    ...
    std::string convert(const std::wstring& wstr)
    {
        ...
        WideCharToMultiByte(CP_UTF8, 0, wstr.data(), static_cast<int>(wstr.size()), str.data(), count, nullptr,
                            nullptr);
    ...
    {
        str.erase(str.begin(),
                  std::find_if(str.begin(), str.end(), [](const unsigned char input) { return !std::isspace(input); }));
