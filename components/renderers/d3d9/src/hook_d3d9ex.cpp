...
namespace gameoverlay
{
	...
	void hook_d3d9ex::reset_handler(void* device)
	{
		if (!device) return;
	...
	void hook_d3d9ex::on_frame(std::function<void(void*)> callback)
	{
		this->frame_callback = callback;
	}

	void hook_d3d9ex::on_reset(std::function<void(void*)> callback)
	{
